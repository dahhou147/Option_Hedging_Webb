    ...
    def get_dividend_yield(self):
        ...
        try:
            ...
            if "dividendYield" in info.columns:
                return (
                    float(info["dividendYield"].iloc[0]) / 100.0
                    if not pd.isna(info["dividendYield"].iloc[0])
                    else 0.0
                )
        except Exception:
...
class IvBlackScholes(DataManager):
    def __init__(
        self, ticker, recalculate_iv=True, min_volume=10, max_spread_ratio=0.5
    ):
    ...
    def preprocessing(self):
        ...
        if "volume" in df_filtered.columns:
            ...
            logger.info(
                f"Filtered by volume >= {self.min_volume}: {len(df_filtered)} options remaining"
            )

        if "bid" in df_filtered.columns and "ask" in df_filtered.columns:
            df_filtered = df_filtered[
                (df_filtered["ask"] - df_filtered["bid"]) / df_filtered["ask"]
                <= self.max_spread_ratio
            ]
            logger.info(
                f"Filtered by spread <= {self.max_spread_ratio * 100}%: {len(df_filtered)} options remaining"
            )
    ...
    def get_implied_vol(self, strike, time_to_expiry, market_price, option_type):
        ...
        try:
            ...
            logger.debug(
                f"Error calculating IV for K={strike}, T={time_to_expiry}: {e}"
            )
            return np.nan
    ...
    def apply_kernel(self):
        ...
        return (
            df.assign(spot_price=self.spot_price)
            .assign(moneyness=lambda x: x["strike"] / x["spot_price"])
            .assign(log_moneyness=lambda x: np.log(x["strike"] / x["spot_price"]))
        )
    ...
    def validate_data(self):
        ...
        if extreme_low > 0 or extreme_high > 0:
            logger.warning(
                f"Extreme volatility values: {extreme_low} too low, {extreme_high} too high"
            )
    ...
    def interpolate_surface_multimethod(self, grid_size=50):
        ...
        if data_density > 10:
            ...
            sigma = 1.5
        logger.info(
            f"Data density: {data_density:.2f}, using sigma={sigma} for smoothing"
        )
    ...
    def detect_and_fill_gaps(self, max_extrapolation_ratio=0.1):
        ...
        logger.info(
            "Available data points: %d strikes × %d maturities",
            len(strikes),
            len(maturities),
        )
...
if __name__ == "__main__":
    iv_surface = DataManager(ticker="AAPL")
    print(iv_surface.data)
    print(iv_surface.options)
    print(iv_surface.free_rate)
    print(iv_surface.spot_price)
    print(iv_surface.dividend_yield)